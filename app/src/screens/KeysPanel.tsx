...
import { KEYS_I18N } from '../ui/i18n/keys';
...
export default function KeysPanel({ onClose, onAccountDeleted }: { onClose: () => void; onAccountDeleted?: () => void }) {
  // Sprache der Oberfläche, nicht des Dokuments — das Konto gehört dem Nutzer.
  const t = KEYS_I18N[useUiLang()];
  ...
  async function saveEmail() {
    ...
    try {
      ...
      setEmailMsg({ kind: 'ok', text: user.email ? t.emailSaved : t.emailRemoved });
    } catch (e) { setEmailMsg({ kind: 'err', text: e instanceof Error ? e.message : t.genericError }); }
  ...
  async function toggleTfa() {
    ...
    try {
      ...
      setTfa({ ...tfa, enabled });
    } catch (e) { setTfaErr(e instanceof Error ? e.message : t.genericError); }
    finally { setTfaBusy(false); }
  }

  async function deleteAccount() {
    setDelErr(null);
    if (delConfirm !== t.deleteConfirmWord) return setDelErr(t.deleteTypeConfirm);
    if (!delPw) return setDelErr(t.deletePasswordMissing);
    setDelBusy(true);
    try {
      await api.deleteMe(delPw);
      onAccountDeleted?.();
    } catch (e) { setDelErr(e instanceof Error ? e.message : t.genericError); }
    finally { setDelBusy(false); }
  }

  async function changePassword() {
    setPwMsg(null);
    if (newPw.length < 8) return setPwMsg({ kind: 'err', text: t.pwTooShort });
    if (newPw !== newPw2) return setPwMsg({ kind: 'err', text: t.pwMismatch });
    setPwBusy(true);
    try {
      await api.changePassword(curPw, newPw);
      setPwMsg({ kind: 'ok', text: t.pwChanged });
      setCurPw(''); setNewPw(''); setNewPw2('');
    } catch (e) { setPwMsg({ kind: 'err', text: e instanceof Error ? e.message : t.genericError }); }
  ...
  const inputLabel: React.CSSProperties = { fontSize: '10.5px', fontWeight: 700, color: 'oklch(0.42 0.017 264)', marginBottom: '4px', fontFamily: UI, display: 'block' };
      ...
      <div data-modal="true" style={{ background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '520px', maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '22px 26px 16px', borderBottom: '1px solid oklch(0.91 0.005 264)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'oklch(0.52 0.012 264)', fontFamily: UI }}>{t.subtitle}</div>
          </div>
        ...
        <div style={{ padding: '18px 26px 28px' }}>
          <div style={sectionH}>{t.sectionPassword}</div>
          <div style={card}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div><label style={inputLabel}>{t.currentPassword}</label><input type="password" value={curPw} onChange={e => setCurPw(e.target.value)} style={input} autoComplete="current-password" /></div>
              <div><label style={inputLabel}>{t.newPassword}</label><input type="password" value={newPw} onChange={e => setNewPw(e.target.value)} style={input} autoComplete="new-password" placeholder={t.newPasswordHint} /></div>
              <div><label style={inputLabel}>{t.newPasswordRepeat}</label><input type="password" value={newPw2} onChange={e => setNewPw2(e.target.value)} style={input} autoComplete="new-password" /></div>
              {pwMsg && (
              ...
              <button type="button" onClick={changePassword} disabled={pwBusy || !curPw || !newPw}
                ...
                {pwBusy ? t.changingPassword : t.changePassword}
              </button>
            </div>
          </div>

          {/* ── Sicherheit: E-Mail + Zwei-Faktor ───────────────────────── */}
          <div style={{ ...sectionH, marginTop: '28px' }}>{t.sectionSecurity}</div>
          <div style={card}>
            <label style={inputLabel}>{t.emailLabel}</label>
            <div style={{ fontSize: '11.5px', color: 'oklch(0.52 0.012 264)', lineHeight: 1.5, marginBottom: '8px', fontFamily: UI }}>
              {t.emailHint}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', justifyContent: 'space-between' }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: 'oklch(0.21 0.021 264)', fontFamily: UI }}>{t.tfaTitle}</div>
                <div style={{ fontSize: '11.5px', color: 'oklch(0.52 0.012 264)', lineHeight: 1.5, marginTop: '3px', fontFamily: UI }}>
                  {t.tfaHint}
                  {tfa && !tfa.canEnable && !tfa.enabled && (
                    <span style={{ color: '#a1322b' }}> {tfa.hasEmail ? t.tfaNoMailer : t.tfaNeedsEmail}</span>
                  )}
                </div>
              </div>
          ...
          {/* ── Verbundene KI-Clients (OAuth/MCP) ───────────────────────── */}
          <div style={{ ...sectionH, marginTop: '28px' }}>{t.sectionApps}</div>
          <div style={card}>
            <div style={{ fontSize: '12.5px', color: 'oklch(0.42 0.017 264)', lineHeight: 1.55, marginBottom: '12px', fontFamily: UI }}>
              {t.appsIntro}
            </div>

            {/* Die Adresse. Ohne sie ist dieser Abschnitt eine Liste, zu der
                niemand etwas hinzufügen kann. */}
            <label style={{ ...inputLabel, marginTop: '2px' }}>{t.mcpLabel}</label>
            <div style={{ display: 'flex', gap: '6px', alignItems: 'stretch' }}>
              ...
              <button type="button" onClick={() => kopieren('url', mcpUrl)}
                ...
                {kopiert === 'url' ? t.copied : t.copy}
              </button>
            </div>
            <div style={{ fontSize: '11px', color: 'oklch(0.52 0.012 264)', lineHeight: 1.5, marginTop: '6px', fontFamily: UI }}>
              {t.mcpHint}
            </div>

            <div style={{ display: 'flex', gap: '6px', marginTop: '10px', flexWrap: 'wrap' }}>
              {([['claude', t.clientClaude], ['codex', t.clientCodex], ['cursor', t.clientCursor]] as const).map(([id, label]) => (
                <button key={id} type="button" onClick={() => setClientHilfe(clientHilfe === id ? null : id)}
                  style={{ padding: '5px 10px', background: clientHilfe === id ? 'oklch(0.21 0.021 264)' : '#fff', color: clientHilfe === id ? '#fff' : 'oklch(0.42 0.017 264)', border: '1px solid oklch(0.87 0.006 264)', borderRadius: '999px', fontSize: '11.5px', fontWeight: 600, cursor: 'pointer', fontFamily: UI }}>
                  {label}
                </button>
              ))}
            </div>
            ...
            {clientHilfe && (
              <div style={{ marginTop: '10px', background: '#fff', border: '1px solid oklch(0.91 0.005 264)', borderRadius: '8px', padding: '11px 13px', fontSize: '12px', color: 'oklch(0.42 0.017 264)', lineHeight: 1.6, fontFamily: UI }}>
                {clientHilfe === 'claude' && (
                  <>
                    <strong>{t.clientClaude}</strong> — {t.claudeHelp}
                  </>
                )}
                {clientHilfe === 'codex' && (
                  <>
                    <strong>{t.clientCodex}</strong> — {t.codexHelpIn} <code style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>~/.codex/config.toml</code>:
...
{`[mcp_servers.heidrich-cv]\nurl = "${mcpUrl}"`}
                    ...
                    {t.codexHelpThen} <code style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>codex mcp login heidrich-cv</code> {t.codexHelpOpens}
                    <button type="button" onClick={() => kopieren('codex', `[mcp_servers.heidrich-cv]\nurl = "${mcpUrl}"`)}
                      style={{ display: 'block', marginTop: '7px', padding: '4px 9px', background: kopiert === 'codex' ? '#2e7d32' : '#fff', color: kopiert === 'codex' ? '#fff' : 'oklch(0.42 0.017 264)', border: '1px solid oklch(0.87 0.006 264)', borderRadius: '6px', fontSize: '11px', fontWeight: 600, cursor: 'pointer', fontFamily: UI }}>
                      {kopiert === 'codex' ? t.copied : t.copySection}
                    </button>
                  </>
                )}
                {clientHilfe === 'cursor' && (
                  <>
                    <strong>{t.cursorHelpTitle}</strong> — {t.cursorHelpWhere} <em>URL</em> {t.cursorHelpOr} <em>Streamable HTTP</em> {t.cursorHelpBelongs} <code style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>mcp/</code> {t.cursorHelpReadme}
                  </>
                )}
              </div>
            )}
            ...
            {conns === null && (
              <div style={{ fontSize: '12px', color: 'oklch(0.52 0.012 264)', fontFamily: UI }}>{t.loading}</div>
            )}
            {conns?.length === 0 && (
              <div style={{ fontSize: '12px', color: 'oklch(0.52 0.012 264)', fontFamily: UI, marginTop: '10px' }}>
                {t.noConnections}
              </div>
            )}
            {conns?.map(c => (
              <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', justifyContent: 'space-between', padding: '9px 0', borderTop: '1px solid oklch(0.94 0.004 264)' }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: c.revoked ? 'oklch(0.52 0.012 264)' : 'oklch(0.21 0.021 264)', fontFamily: UI, textDecoration: c.revoked ? 'line-through' : 'none' }}>
                    {c.client_name || t.unnamedClient}
                  </div>
                  <div style={{ fontSize: '11px', color: 'oklch(0.52 0.012 264)', fontFamily: UI, marginTop: '2px' }}>
                    {t.connected} {new Date(c.created_at).toLocaleDateString(t.dateLocale)}
                    {c.last_used_at ? ` · ${t.lastActive} ${new Date(c.last_used_at).toLocaleDateString(t.dateLocale)}` : ` · ${t.neverUsed}`}
                  </div>
                </div>
                {!c.revoked && (
          ...
          {/* ── DSGVO: Datenexport ──────────────────────────────────────── */}
          <div style={{ ...sectionH, marginTop: '28px' }}>{t.sectionData}</div>
          <div style={card}>
            <div style={{ fontSize: '12.5px', color: 'oklch(0.42 0.017 264)', lineHeight: 1.55, marginBottom: '12px', fontFamily: UI }}>
              {t.dataIntro}
            </div>
          ...
          {/* ── DSGVO: Konto löschen ────────────────────────────────────── */}
          <div style={{ ...sectionH, marginTop: '28px', color: '#a1322b' }}>{t.sectionDelete}</div>
          <div style={{ ...card, borderColor: '#f0c0c0', background: '#fff5f3' }}>
            ...
            {!delOpen ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div>
                  <label style={inputLabel}>{t.currentPassword}</label>
                  <input type="password" value={delPw} onChange={e => setDelPw(e.target.value)} style={input} autoComplete="current-password" />
                </div>
                <div>
                  <label style={inputLabel}>{t.deleteConfirmBefore} <b>{t.deleteConfirmWord}</b> {t.deleteConfirmAfter}</label>
                ...
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                  <button type="button" onClick={() => { setDelOpen(false); setDelErr(null); setDelPw(''); setDelConfirm(''); }} disabled={delBusy}
                    style={{ padding: '8px 14px', background: 'transparent', border: '1px solid oklch(0.87 0.006 264)', borderRadius: '7px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', color: '#666', fontFamily: UI }}>{t.cancel}</button>
                  <button type="button" onClick={deleteAccount} disabled={delBusy || delConfirm !== t.deleteConfirmWord}
                    ...
                    {delBusy ? t.deleting : t.deleteFinal}
                  </button>
                </div>
              </div>
            )}
          ...
          <div style={{ marginTop: '24px', fontSize: '11.5px', color: 'oklch(0.52 0.012 264)', fontFamily: UI, lineHeight: 1.6 }}>
            <b>{t.tipLabel}</b> {t.tipBody}
          </div>
        </div>
      </div>
    </div>
