...
import { RESET_I18N } from '../ui/i18n/klein';
...
const SOFT = 'oklch(0.42 0.017 264)';
const FAINT = 'oklch(0.52 0.012 264)';
...
export default function ResetPassword({ token }: { token: string }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (pw.length < 8) { setError(T.tooShort); return; }
    if (pw !== pw2) { setError(T.mismatch); return; }
    setBusy(true);
    try {
    ...
    <div style={{ minHeight: '100vh', background: PAPER, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: SANS, color: INK, padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ background: SURFACE, border: `1px solid ${LINE}`, borderRadius: '16px', boxShadow: '0 6px 20px -8px oklch(0.21 0.02 264 / 0.14)', padding: '30px' }}>
          {done ? (
            <>
              <div style={{ fontFamily: DISP, fontSize: '26px', fontWeight: 600, letterSpacing: '-0.03em', marginBottom: '10px' }}>{T.doneTitle}</div>
              <p style={{ fontSize: '14px', color: SOFT, lineHeight: 1.55, marginBottom: '22px' }}>{T.doneBody}</p>
              <a href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: '9px', background: ACCENT, color: '#fff', textDecoration: 'none', fontWeight: 550, fontSize: '14px', padding: '13px 18px', borderRadius: '9px' }}>{T.toSignIn}</a>
            </>
          ) : (
            <form onSubmit={submit}>
              <div style={{ fontFamily: DISP, fontSize: '26px', fontWeight: 600, letterSpacing: '-0.03em', marginBottom: '8px' }}>{T.title}</div>
              <p style={{ fontSize: '14px', color: SOFT, lineHeight: 1.55, margin: 0 }}>{T.intro}</p>
              <label style={label}>{T.passwordLabel}</label>
              ...
              <label style={label}>{T.repeatLabel}</label>
